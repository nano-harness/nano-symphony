...
describe("worker persists artifacts.patch via tracker", () => {
  test("patch from spawnResult.artifacts is recorded on the run", async () => {
    ...
    tracker.insertIssue({ id: "patch-1", identifier: "PATCH-1", title: "t", state: "todo" });
    ...
    const row = db.query("SELECT last_patch FROM symphony_runs WHERE issue_id = ?").get("patch-1") as { last_patch: string | null };
    expect(row.last_patch).toBe(fakePatch);

    // Also verify patch_collected event was emitted
    const events = tracker.getEvents().filter((e) => e.issue_id === "patch-1" && e.kind === "patch_collected");
    expect(events.length).toBe(1);
  });

  test("no patch field leaves last_patch as NULL", async () => {
    const { tracker, db } = mkTracker();
    tracker.insertIssue({ id: "patch-2", identifier: "PATCH-2", title: "t", state: "todo" });
    const wsRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nano-symphony-patch-"));
    ...
    const row = db.query("SELECT last_patch FROM symphony_runs WHERE issue_id = ?").get("patch-2") as { last_patch: string | null };
